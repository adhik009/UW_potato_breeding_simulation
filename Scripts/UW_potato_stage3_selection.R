...
for(REP in 1:5) {
  ...
  for(year in 21:40){
    ...
    Stage3_parents = selectInd(Stage3, nInd = 20, use = "pheno", selectTop = TRUE)
    ...
    # create parent pool
    # Parents from previous year and Stage3_parents of this year is appended in parentpool
    ParentPool <- append(Parents, Stage3_parents) 
  ...
  output = tibble(scenario=rep("Stage3_sel",40),
                  year=-19:20,
  ...
  saveRDS(output,paste0("Stage3_sel_",REP,".rds"))
