...
# Changing crossing strategy to Stage3 by Stage2
...
for(REP in 1:5) {
  ...
  for(year in 21:40){
    ...
    Stage4 = setPheno(Stage4, reps=2, varE=1.30)
    ...
    Stage1_parents = selectInd(Stage1, nInd = 20, selectTop = TRUE)
    
    # create parent pool
    # Parents from previous year and Stage1 of this year is appended in parentpool
    ParentPool <- append(Parents, Stage1_parents) 
  ...
  output = tibble(scenario=rep("Stage1_sel",40),
                  year=-19:20,
  ...
  saveRDS(output,paste0("Stage1_sel_",REP,".rds"))
}
