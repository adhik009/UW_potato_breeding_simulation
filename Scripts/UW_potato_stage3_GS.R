...
for(REP in 1:5) {
  ...
  for(year in 21:40){
    ...
    trainPop <- append(trainPop[200:1000], Stage3) #explore different lag times for updating the model
    gs = RRBLUP(pop=trainPop, use = "pheno", useQtl = FALSE)
    
    ParentPool <- append(Stage3, Parents)
  ...
  output = tibble(scenario=rep("Stage3_GS",40),
                  year=-19:20,
  ...
  saveRDS(output,paste0("Stage3_GS_",REP,".rds"))
